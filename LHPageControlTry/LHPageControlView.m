...
//        _topHidenHightLightCircles.position = CGPointMake(- (kPageWidth + 5) * distance , 0);
    ...
    {
    //    _hightLightView.centerX = 25 + distance*(kPageWidth+5);
        //---移动高亮层的动画
        _hightLightView.frame = CGRectMake(20+distance*(kPageHeight+5),(self.height - kPageHeight)/2, kPageWidth, kPageHeight);
        _topHidenHightLightCircles.frame = CGRectMake(- (kPageWidth + 5) * distance , 0 , kPageWidth, kPageHeight);
        ...
        if (scale >= 0 && scale <= 0.2) {
            //缩小
            _hightLightView.width = kPageWidth*(1.0-scale);
            _hightLightView.height = kPageHeight*(1.0-scale);
            //调整中心
            _hightLightView.centerX = 25 + distance*(kPageWidth+5);
            _hightLightView.centerY = self.height/2.0;
            
            _hightLightView.layer.cornerRadius = _hightLightView.width/2;

        }
        
        //---移动的范围 （ 0.2 ~ 0.8 ）之间，缩小大小到 0.8 倍
        if (scale > 0.2 && scale < 0.8) {
            //缩小
            _hightLightView.width = kPageWidth*0.8;
            _hightLightView.height = kPageHeight*0.8;
            //调整中心
            _hightLightView.centerX = 25 + distance*(kPageWidth+5);
            _hightLightView.centerY = self.height/2.0;
            
            _hightLightView.layer.cornerRadius = kPageWidth*0.8/2;
        }
        
        //---移动的范围 （ 0 ~ 0.2 ）之间，放大大小
        if (scale >= 0.8 && scale <= 1.0) {
            
            _hightLightView.width = kPageWidth*scale;
            _hightLightView.height = kPageHeight*scale;
